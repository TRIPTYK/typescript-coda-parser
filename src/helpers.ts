...
function getTrimmedData (data:string, startPosition:number, length?:number) {
  if (length) {
...
function getLinesOfType<T extends Line> (lines:Line[], type:Class<T>):T[] | undefined {
  return lines.filter(line => line.constructor === type) as T[];
};
...
export { getLinesOfType, validateStringDigitOnly, validateStringLength, validateStringMultipleLengths, formatDateString, getTrimmedData, getFirstLineOfType, filterLinesOfTypes }
